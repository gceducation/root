...
namespace EduDB
{
    public class RandomDataGenerator
    {
        ...
        public String getStringByPattern(String sPattern)
        {
            char[] ach = new char[sPattern.Length];
            int jch;
            for (jch = 0; jch < ach.Length; jch++)
            {
                char chPat = sPattern[jch];
                char ch = '\0';
                switch(chPat)
                {
                    case '~': // A-Z character
                        ch = (char)(_random.Next(26) + 'A');
                        break;

                    case '`': // number
                        ch = (char)(_random.Next(10) + '0');
                        break;
                    default:
                        ch = chPat;
                        break;
